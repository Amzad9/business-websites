...
function fixDateRendering(filePath) {
  try {
    let content = fs.readFileSync(filePath, 'utf8');
    const originalContent = content;
    let updated = false;

    // Pattern 1: Simple date rendering {post.frontmatter.date}
    // Make sure it doesn't already have the fix
    if (content.includes('{post.frontmatter.date}') && !content.includes('instanceof Date')) {
      // Replace on same line
    ...
    // Pattern 2: Date with type cast {post.frontmatter.date as string}
    ...
    // Pattern 3: Date in paragraph tag on same line
    if (content.includes('<p') && content.includes('post.frontmatter.date') && !content.includes('instanceof Date')) {
      // Multi-line replacement for better formatting
      content = content.replace(
        /(<p[^>]*>)\s*\{post\.frontmatter\.date(?:\s+as\s+string)?\}\s*(<\/p>)/g,
        `$1
                          {$2 instanceof Date 
                            ? $2.toLocaleDateString() 
                            : String($2 || '')}
                        $3`.replace(/\$2/g, 'post.frontmatter.date')
      );
      // Fix the replacement - let's do it more carefully
      content = content.replace(
        /(<p[^>]*className="[^"]*"[^>]*>)\s*\{post\.frontmatter\.date(?:\s+as\s+string)?\}\s*(<\/p>)/g,
        `$1
                          {post.frontmatter.date instanceof Date 
                            ? post.frontmatter.date.toLocaleDateString() 
                            : String(post.frontmatter.date || '')}
...
console.log('🔧 Fixing date rendering in all blog pages...\n');
...
businessNames.forEach((siteName, index) => {
  ...
  if (fs.existsSync(blogPagePath)) {
    if (fixDateRendering(blogPagePath)) {
      fixedCount++;
      siteUpdated = true;
    }
  }
  ...
  if (fs.existsSync(blogSlugPagePath)) {
    if (fixDateRendering(blogSlugPagePath)) {
      fixedCount++;
      siteUpdated = true;
    }
  }
  ...
  if (siteUpdated) {
    ...
    skippedCount++;
    // Only log every 20th to avoid spam
    if (index % 20 === 0 || index === businessNames.length - 1) {
